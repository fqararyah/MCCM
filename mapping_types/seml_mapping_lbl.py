...
class SEMLMapping_LBL(SimpleMapping):
    ...
    def calc_fms_buffer_sz_intra(self):
        ...
        if fms_buffer_sz == 0:
            fms_buffer_sz = min(2, self.hw_config.on_chip_memory - weights_buffer_sz)
    ...
    def calc_weights_buffer_sz(self):
        ...
        for i in range(len(self.layers)):
            ...
            if utils.is_dw_layer(layer_specs):
                weights_buffer_sz = max(weights_buffer_sz,
                                        ...
                                        self.engines[self.dw_engine_index].par_ofms)
            else:
                dw_weights_buffer_sz = max(weights_buffer_sz,
                                           utils.get_layer_ifms_shape(layer_specs)[0] * filter_dim * filter_dim *
        ...
        if len(self.engines) > 1:
            dw_weights_buffer_sz = self.calc_actual_bram_cons(
                dw_weights_buffer_sz, self.engines[self.dw_engine_index].get_ports_weights())
    ...
    def calc_off_chip_fms_and_weights_access_intra(self):
        weights_buffer_sz = self.calc_weights_buffer_sz()
        ifms_buffer_sz_intra = self.calc_fms_buffer_sz_intra() / 2
        for i in range(len(self.layers)):
            layer_index = self.layers[i]
            layer_specs = self.model_dag[layer_index]
            ...
            ifm_ofm_size = max(layer_ifms_sz_act, layer_ofms_sz_act) * 2
            if ifm_ofm_size >= self.hw_config.on_chip_memory - weights_buffer_sz:
                os_l_ifms_stationary_weight_accesses = layer_weights_size * \
                    math.ceil(layer_ifms_sz_act / ifms_buffer_sz_intra)
                os_l_weights_stationary_ifms_accesses = layer_ifms_sz * \
                    math.ceil(layer_weights_size / weights_buffer_sz)
